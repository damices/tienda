    ...
    def guardarUsuario (self):
        if self.txtContraseña.get() == self.txtRepetirContraseña.get():
            ...
            if Conexion.run_query(self.sql, self.parametros):
               messagebox.showinfo(message="Datos guardados corectamente", title="Guardar datos")
            else:
                messagebox.showwarnig(message="Nose pueden guardar datos", title="Error al guardar datos")
                self.ventanaUsuarios.destroy()
        else:
            messagebox.showinfo(message="la contraseña son son iguales", title="Validar contraseña")
